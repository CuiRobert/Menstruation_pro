...
parser.add_argument('--lr',type=float,default=8e-5,metavar='LR', #0.0002
...
parser.add_argument('--trainlabel-dir',type=str,default='../Data/训练集——FZ——其他切面（全部数据）.xlsx',metavar='Label',
...
parser.add_argument('--testlabel-dir',type=str,default='../Data/测试集——FZ——其他切面(一一对应).xlsx',metavar='Label',
...
save_dir = os.path.join('../weight_models_QT/',args.model_name,)
...
def get_logger(filename, verbosity=1, name=None):
    if not os.path.isdir('../weight_models_QT'):
        os.makedirs('../weight_models_QT')
...
logger = get_logger(save_dir+'/{}_{}_{}.log'.format(args.model_label,args.label_dir.replace('../Data/','').replace('.xlsx',''),  save_label))
...
def test(model, test_loader,classes, criterion = nn.CrossEntropyLoss(weight=torch.FloatTensor([1,2]).to(device))):
    ...
    df = pd.DataFrame(c)
    df.to_excel(save_dir+'/' + args.model_label+ save_label + args.label_dir.replace('../Data/',''),index=False)
